...
const UpdateDetails = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5001/user/get-details/${localStorage.getItem(
            "userID"
          )}`
        );
  ...
  const handleSaveChanges = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:5001/user/update-details/${localStorage.getItem(
          "userID"
        )}`,
        {
          method: "PUT",
      ...
      if (response.status === 200) {
        if (result.emailChanged) {
          setWarning(
            `An verification email sent to ${userData.email}, please verify your updated email address.`
          );
        } else {
          setWarning("");
        }
        alert(`User details updated successfully! ${warning}`);
        navigate("/user/home");
      } else {
